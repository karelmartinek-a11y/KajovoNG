...
class CascadePanel(QWidget):
    def __init__(self, settings, model_provider, parent=None):
        ...
        # HEADER: název + správa uložených kaskád + OUT directory
        header = QWidget()
        ...
        row1 = QHBoxLayout()
        row1.setSpacing(8)
        row1.addWidget(QLabel("Název kaskády"))
        self.ed_name = QLineEdit("nova_kaskada")
        row1.addWidget(self.ed_name, 2)

        row1.addWidget(QLabel("Uložené"))
        self.cb_saved = QComboBox()
        self.cb_saved.setMinimumWidth(260)
        row1.addWidget(self.cb_saved, 1)
        ...
        row1.addWidget(self.btn_refresh_saved)
        ...
        row2 = QHBoxLayout()
        row2.setSpacing(8)
        row2.addWidget(QLabel("OUT adresář (default pro Kaskádu)"))
        self.ed_default_out_dir = QLineEdit()
        self.btn_default_out_browse = QPushButton("Browse")
        row2.addWidget(self.ed_default_out_dir, 1)
        row2.addWidget(self.btn_default_out_browse)
        header_l.addLayout(row2)

        root.addWidget(header)

        # BODY: 2 sloupce – vlevo kroky, vpravo editor vybraného kroku
        split = QSplitter(Qt.Orientation.Horizontal)
        split.setChildrenCollapsible(False)
        root.addWidget(split, 1)

        # LEFT: seznam kroků + akce
        left = QWidget()
        lv = QVBoxLayout(left)
        lv.setContentsMargins(4, 4, 4, 4)
        lv.setSpacing(8)

        lv.addWidget(QLabel("Kroky"))
        ...
        lv.addWidget(self.lst_steps, 1)

        actions = QHBoxLayout()
        actions.setSpacing(6)
        self.btn_add_step = QPushButton("Přidat")
        self.btn_delete_step = QPushButton("Smazat")
        self.btn_duplicate_step = QPushButton("Duplikovat")
        self.btn_move_step_up = QPushButton("↑")
        self.btn_move_step_down = QPushButton("↓")
        ...
        # RIGHT: editor kroku (scroll)
        right = QWidget()
        rv = QVBoxLayout(right)
        rv.setContentsMargins(4, 4, 4, 4)
        rv.setSpacing(8)

        from PySide6.QtWidgets import QScrollArea, QSizePolicy, QFrame  # lokální import kvůli kompatibilitě
        ...
        editor_l.setSpacing(10)
        scroll.setWidget(editor)

        # META
        box_meta = QGroupBox("Nastavení kroku")
        ...
        editor_l.addWidget(box_meta)

        # MAIN CONTENT: vlevo texty, vpravo soubory + výstup
        inner = QSplitter(Qt.Orientation.Horizontal)
        inner.setChildrenCollapsible(False)
        editor_l.addWidget(inner, 1)

        # INPUTS (větší plocha)
        w_inputs = QWidget()
        il = QVBoxLayout(w_inputs)
        il.setContentsMargins(0, 0, 0, 0)
        il.setSpacing(8)

        box_input = QGroupBox("Obsah (Inputs)")
        ...
        self.txt_input_text.setPlaceholderText("Input text (použije se jen když je Input Content JSON prázdné)")
        self.txt_input_text.setMinimumHeight(140)

        self.txt_input_content = QPlainTextEdit()
        self.txt_input_content.setPlaceholderText("Input content parts JSON (Responses API list/object, má prioritu)")
        self.txt_input_content.setMinimumHeight(160)
        ...
        il.addWidget(box_input, 1)
        inner.addWidget(w_inputs)

        # RIGHT SIDEBAR: files + output schema (+ skryté QFILE-like)
        w_side = QWidget()
        sl = QVBoxLayout(w_side)
        sl.setContentsMargins(0, 0, 0, 0)
        sl.setSpacing(8)

        box_files = QGroupBox("Soubory (Files)")
        ...
        fv.addLayout(fr)
        sl.addWidget(box_files, 1)

        box_out = QGroupBox("Výstup")
        ...
        sl.addWidget(box_out)

        # Expected output files (QFILE-like) – záměrně skryto, OUT řeší jen adresář nahoře.
        self.grp_expected = QGroupBox("Expected output files (QFILE-like)")
        self.grp_expected.setVisible(False)
        ev = QVBoxLayout(self.grp_expected)
        ...
        sl.addWidget(self.grp_expected)

        inner.addWidget(w_side)
        inner.setStretchFactor(0, 3)
        inner.setStretchFactor(1, 2)
        inner.setSizes([860, 560])
        ...
        rv.addLayout(editor_buttons)

        split.addWidget(right)
        split.setStretchFactor(0, 1)
        split.setStretchFactor(1, 3)
        split.setSizes([420, 1120])

        # SIGNALS        self.btn_add_step.clicked.connect(self.add_step)
