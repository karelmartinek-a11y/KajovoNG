...
def _record_pending_lineage(window, before: set[str]) -> None:
    ...
    def finish_lineage():
        ...
        if logger_value is not None and isinstance(getattr(window, "_history_pending_lineage", None), dict):
            try:
                logger_value.record_lineage(**window._history_pending_lineage)
                window._history_pending_lineage = None
                window.history_panel.refresh_runs()
...
def rerun_as_new(window, run_id: str) -> None:
    try:
        ...
        checkpoints = [
            item for item in adapter.checkpoints() if item.get("safe_to_continue")
        ]
        if not checkpoints:
            raise ValueError(
...
def install_history_run_explorer(window):
    """Připojí nové akce k MainWindow bez změny starých běhů nebo vytvoření větve."""
    ...
    # MainWindow ze staršího UI připojuje `rerun` na starou recovery cestu.
    # Runtime instalace ji nahradí novou semantikou: nový run + lineage.
    try:
    ...
    # Tlačítko Spustit práce musí spotřebovat případnou připravenou lineage
    # z Klonovat / Použít znovu. Původní on_go zůstává jediným místem startu workflow.
    try:
    ...
    # Nové zadání ruší pouze čekající lineage; již uložené Run Bundle se nemění.
