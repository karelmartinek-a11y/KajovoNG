...
from ..core.run_bundle import HistoryIndex, LegacyRunAdapter, RunBundle
from .batch_view import history_batch_detail, project_name
from .design import button, column, editor, label, row, table, text
...
class TechnicalViewer(QWidget):
    """Raw viewer s hledáním; obsah evidence se nijak nemění ani nerediguje."""
...
class StepDetailDialog(QDialog):
    """Detail jednoho logického kroku nad stejnou kanonickou evidencí."""
    ...
    def __init__(self, adapter: LegacyRunAdapter, step: dict, parent=None):
        ...
        inputs = TechnicalViewer()
        requests = [
            item for item in adapter.requests() if not step_id or item.get("step_id") == step_id
        ]
        inputs.set_value([item.get("full_payload", item) for item in requests])
        tabs.addTab(inputs, "Vstupy")

        responses = TechnicalViewer()
        response_records = [
            item for item in adapter.responses() if not step_id or item.get("step_id") == step_id
        ]
        responses.set_value(response_records)
        tabs.addTab(responses, "Odpovědi")

        artifacts = TechnicalViewer()
        artifacts.set_value(
            [item for item in adapter.artifacts() if not step_id or item.get("step_id") == step_id]
        )
        tabs.addTab(artifacts, "Soubory")

        validations = TechnicalViewer()
        validations.set_value(
            [item for item in adapter.validations() if not step_id or item.get("step_id") == step_id]
        )
        tabs.addTab(validations, "Validace")

        events = TechnicalViewer()
        events.set_value(
            [item for item in adapter.events() if not step_id or item.get("step_id") == step_id]
        )
        tabs.addTab(events, "Události")
...
class ResponseRequestPanel(QWidget):
    """Dashboard Historie; název třídy zůstává kvůli kompatibilitě volajících modulů."""
    ...
    def _build_ui(self):
        ...
        # Kompatibilní přesné filtry z původní Historie.
        ...
        self.period.addItems(
            ["Vše", "Dnes", "Včera", "Posledních 7 dní", "Posledních 30 dní", "Tento měsíc", "Vlastní datum"]
        )
        self.project_filter = QComboBox()
        self.mode_filter = QComboBox()
        self.status_filter = QComboBox()
        self.model_filter = QComboBox()
        for combo in (self.project_filter, self.mode_filter, self.status_filter, self.model_filter):
            combo.addItem("Vše")
        ...
        # Legacy atributy pro starší UI/testy; seznam reprezentuje response zvoleného běhu.
    ...
    def _build_overview_tab(self):
        ...
        for key, title in (
            ...
            self.overview_form.addRow(title, value)
        body.addLayout(self.overview_form)
    ...
    def _build_timeline_tab(self):
        ...
        self.timeline.setHeaderLabels(
            ["#", "Krok", "Stav", "Čas", "Model", "Reasoning", "Requesty", "Response", "Artefakty", "Checkpoint"]
        )
        ...
        body.addWidget(QLabel("Dvojklikem otevřete vstupy, odpovědi, soubory, validace, události a raw technický detail kroku."))
    ...
    def _build_responses_tab(self):
        ...
        self.response_status_filter.addItems(["Vše", "completed", "incomplete", "failed", "error", "unknown"])
        ...
        self.response_list.currentItemChanged.connect(lambda _now, _old: self.load_selected_entry())
    ...
    def _build_events_tab(self):
        ...
        body.addLayout(filters)
        self.event_table = table(["#", "Čas", "Krok", "Typ", "Severity", "Lidská zpráva"])
    ...
    def _build_lineage_tab(self):
        ...
        self.lineage_tree.setHeaderLabels(["Vztah", "RUN", "Checkpoint", "Čas", "Poznámka"])
    ...
    def _collect_entries(directory):
        """Legacy kompatibilita: nová Historie používá adapter/index, ne tento scan pro filtry."""
        adapter = LegacyRunAdapter(directory)
        entries = []
        for kind, records in (("Požadavek", adapter.requests()), ("Odpověď", adapter.responses())):
            for record in records:
                path = str(record.get("source_path") or "")
                entries.append(
                    {
                        "path": path,
                        "name": Path(path).name if path else str(record.get("response_id") or record.get("request_record_id") or record.get("response_record_id") or kind),
                        "kind": kind,
    ...
    def reset_filters(self):
        ...
        for check in (self.chk_error, self.chk_batch, self.chk_checkpoint, self.chk_output, self.chk_lineage):
    ...
    def _refill_filter_values(self):
        mappings = (
            (self.project_filter, sorted({str(item.get("project") or "") for item in self._runs if item.get("project")})),
            (self.mode_filter, sorted({str(item.get("mode") or "") for item in self._runs if item.get("mode")})),
            (self.status_filter, sorted({str(item.get("status") or "") for item in self._runs if item.get("status")})),
            (self.model_filter, sorted({model for item in self._runs for model in item.get("models", []) if model})),
        )
        for combo, values in mappings:
            ...
            combo.setCurrentText(current if current in values or current == "Vše" else "Vše")
    ...
    def _period_match(self, record: dict) -> bool:
        ...
        if not created:
            _, legacy_date, _timestamp = self._parse_run_date(str(record.get("run_id") or ""))
            if legacy_date:
                try:
                    created = datetime.strptime(legacy_date, "%Y-%m-%d").replace(tzinfo=timezone.utc)
                except ValueError:
                    created = None
        if not created:
            return False
    ...
    def apply_filters(self):
        ...
        for record in self._runs:
            ...
            if query and query not in str(record.get("search_text") or ""):
                continue
            if response_query and not any(response_query in value.casefold() for value in record.get("response_ids", [])):
                continue
            if explicit_date:
                created = _parse_iso(str(record.get("created_at") or ""))
                date_key = created.astimezone().strftime("%Y-%m-%d") if created else self._parse_run_date(record["run_id"])[1]
                if date_key != legacy_date:
                    continue
            if not self._period_match(record):
                continue
            if self.project_filter.currentText() != "Vše" and record.get("project") != self.project_filter.currentText():
                continue
            if self.mode_filter.currentText() != "Vše" and record.get("mode") != self.mode_filter.currentText():
                continue
            if self.status_filter.currentText() != "Vše" and record.get("status") != self.status_filter.currentText():
                continue
            if self.model_filter.currentText() != "Vše" and self.model_filter.currentText() not in record.get("models", []):
                continue
            ...
            title = (
                ...
                f" · kroky {record.get('steps', 0)} · odpovědi {record.get('responses', 0)} · soubory {record.get('artifacts', 0)}"
            )
            ...
            item.setToolTip("Kliknutím otevřete Run Explorer; stará evidence se nikdy při prohlížení nemění.")
            ...
            if pending_batch_ids(state):
                ...
                action.setEnabled(not self.batch_busy and run_id not in self.active_runs)
                action.setToolTip("Převezme výsledky již odeslané pracovní dávky. Nevytváří nový generativní požadavek.")
    ...
    def _update_actions(self):
        ...
        blocked = bool(summary and (self.batch_busy or summary.get("run_id") in self.active_runs))
        state = read_state(Path(self.log_dir) / summary["run_id"]) if summary else {}
        has_sent_batch = bool(batch_ids(state))
        can_continue = has_run and not legacy and checkpoint and not blocked and not has_sent_batch
        can_clone = has_run and not blocked
        can_rerun = can_continue
        ...
        self.btn_overview_rerun.setEnabled(can_rerun)
        self.btn_overview_repair.setEnabled(can_continue)
        if legacy:
            tooltip = "Legacy běh nemá explicitní bezpečný checkpoint; lze jej pouze klonovat, pokud má uložené zadání."
        elif has_sent_batch:
            tooltip = "Již odeslaný BATCH se dokončuje v původním běhu; nevytváří se jeho duplicitní submit."
        elif not checkpoint:
            tooltip = "Běh nemá zvolený bezpečný checkpoint."
        else:
            tooltip = "Vznikne nový navázaný běh; původní evidence zůstane neměnná."
    ...
    def load_entries_for_selected_run(self):
        ...
        self._adapter = LegacyRunAdapter(Path(self.log_dir) / self._selected_run_id)
        for index in range(self.lst_runs.count()):
            item = self.lst_runs.item(index)
    ...
    def render_overview(self):
        ...
        self.overview_title.setText(f"{run.get('project') or UNKNOWN} · {run.get('run_id') or self._selected_run_id}")
        if integrity.get("valid"):
            self.integrity_label.setText("Integrita ověřena · " + str(integrity.get("bundle_hash") or ""))
            self.integrity_label.setStyleSheet("color:#176b3a;font-weight:600;")
        elif integrity.get("status") == "legacy":
            self.integrity_label.setText("Legacy evidence · integrita celého Run Bundle není evidována")
            self.integrity_label.setStyleSheet("color:#7a5c00;font-weight:600;")
        else:
            self.integrity_label.setText("Evidence je neúplná / změněná · " + "; ".join(integrity.get("errors") or []))
            self.integrity_label.setStyleSheet("color:#9f2d20;font-weight:600;")
        inputs = [item.get("display_name") for item in artifacts if item.get("role") in {"user_input", "attached_file", "in_project_file"}]
        outputs = [item.get("display_name") for item in artifacts if item.get("role") in {"generated_file", "modified_file", "batch_output", "log_export"}]
        errors = sum(1 for item in events if str(item.get("severity") or "").lower() == "error")
        ...
        values = {
            ...
            "started": _human_time(str(run.get("started_at") or run.get("created_at") or "")),
            "finished": _human_time(str(run.get("finished_at") or "")),
            "duration": _duration(str(run.get("started_at") or run.get("created_at") or ""), str(run.get("finished_at") or "")),
            ...
            "inputs": ", ".join(str(value) for value in inputs) if inputs else UNKNOWN,
            "outputs": ", ".join(str(value) for value in outputs) if outputs else UNKNOWN,
            "parent": run.get("parent_run_id") or UNKNOWN,
        }
        for key, value in values.items():
            self.overview_fields[key].setText(str(value))
        summary = str(run.get("output_summary") or run.get("input_summary") or "")
        if run.get("legacy"):
            summary = (summary + "\n\nLegacy běh: chybějící informace nejsou doplněny odhadem.").strip()
        ...
        for checkpoint in checkpoints:
            ...
            title = ("✓ " if safe else "○ ") + str(checkpoint.get("checkpoint_type") or checkpoint.get("checkpoint_id") or UNKNOWN)
            self.checkpoint_combo.addItem(title, checkpoint.get("checkpoint_id") if safe else "")
        self.checkpoint_combo.blockSignals(False)
        self.checkpoint_combo.currentIndexChanged.connect(self._update_actions, Qt.UniqueConnection)
    ...
    def render_timeline(self):
        ...
        if not steps and self._adapter.legacy:
            root = QTreeWidgetItem(["", "Legacy běh", UNKNOWN, UNKNOWN, UNKNOWN, UNKNOWN, "0", "0", "0", UNKNOWN])
            root.setToolTip(1, "Starý běh nemá explicitní StepRecord; UI nevytváří kroky odhadem.")
        ...
        for step in steps:
            item = QTreeWidgetItem(
                [
                    ...
                    _duration(str(step.get("started_at") or ""), str(step.get("finished_at") or "")),
        ...
        for column in range(self.timeline.columnCount()):
            self.timeline.resizeColumnToContents(column)
    ...
    def save_selected_entry(self):
        if not self._selected_response:
            return
        path, _ = dialog_save_file(self, "Uložit detail", "response.txt", "Text (*.txt)")
        if path:
            try:
                Path(path).write_text(self.response_human.toPlainText(), encoding="utf-8")
    ...
    def render_artifacts(self):
        ...
        for artifact in self._adapter.artifacts():
            ...
            values = [
                ...
                str(artifact.get("size_bytes") if artifact.get("size_bytes") is not None else UNKNOWN),
            ...
            for column, value in enumerate(values):
                ...
                self.artifact_table.setItem(row_index, column, cell)
        self.artifact_table.resizeColumnsToContents()

    def _artifact_selected(self):
        selected = self.artifact_table.selectionModel().selectedRows() if self.artifact_table.selectionModel() else []
        if not selected:
            self._selected_artifact = None
            return
    ...
    def preview_artifact(self):
        ...
        path = self._artifact_path(self._selected_artifact)
        if not path:
            self.artifact_preview.set_value({"stav": "Soubor již není lokálně dostupný.", "artefakt": self._selected_artifact})
            return
        try:
            ...
            try:
                ...
                content = f"Binární soubor · {path.stat().st_size} B · SHA-256 {self._selected_artifact.get('sha256') or UNKNOWN}"
            if truncated:
    ...
    def save_artifact_as(self):
        ...
        if not source:
            msg_warning(self, "Artefakt", "Soubor již není lokálně dostupný.")
            return
        target, _ = QFileDialog.getSaveFileName(self, "Uložit artefakt", source.name)
        if target:
            try:
    ...
    def reuse_selected_artifacts(self):
        if not self._adapter:
            return
        rows = self.artifact_table.selectionModel().selectedRows() if self.artifact_table.selectionModel() else []
        identifiers = []
        for index in rows:
            item = self.artifact_table.item(index.row(), 0)
            artifact = item.data(Qt.UserRole) if item else None
            if isinstance(artifact, dict) and artifact.get("reusable") and artifact.get("artifact_id"):
                identifiers.append(artifact["artifact_id"])
        if not identifiers:
            msg_info(self, "Použít soubory", "Vyberte alespoň jeden znovupoužitelný artefakt nového Run Bundle.")
            return
    ...
    def render_events(self):
        ...
        for event in self._adapter.events():
            ...
            values = [
                ...
                event_severity,
                event.get("human_message") or _short(str((event.get("data") or {}).get("msg") or "")) or UNKNOWN,
            ]
            for column, value in enumerate(values):
                cell = QTableWidgetItem(str(value))
                cell.setData(Qt.UserRole, event)
                self.event_table.setItem(row_index, column, cell)
    ...
    def render_lineage(self):
        ...
        if parent:
            item = QTreeWidgetItem(["Rodič", parent, str(run.get("continued_from_checkpoint_id") or ""), "", str(run.get("lineage_reason") or "")])
        ...
        for record in self._adapter.lineage():
            ...
            item.setData(0, Qt.UserRole, {"run_id": source, "record": record})
            self.lineage_tree.addTopLevelItem(item)
        for child in self._reverse_lineage.get(self._selected_run_id, []):
            ...
            item.setData(0, Qt.UserRole, {"run_id": target, "record": child})
            self.lineage_tree.addTopLevelItem(item)
        self.lineage_tree.itemSelectionChanged.connect(self._lineage_selected, Qt.UniqueConnection)
    ...
    def _lineage_open(self, item, _column=0):
        ...
        for index in range(self.lst_runs.count()):
            ...
            if isinstance(summary, dict) and summary.get("run_id") == run_id:
                ...
                return
        msg_info(self, "Návaznost", f"Běh {run_id} je mimo aktuální filtr. Resetujte filtry pro jeho otevření.")

    def render_technical(self):
        if not self._adapter:
            return
        root = self._adapter.root
        self.raw_bundle.set_value(json.loads((root / "bundle.json").read_text(encoding="utf-8")) if (root / "bundle.json").exists() else {"stav": "Legacy běh", "bundle.json": UNKNOWN})
    ...
    def _continue(self):
        ...
        if self._selected_run_id and checkpoint_id and self.btn_continue.isEnabled():
    ...
    def _repair(self):
        checkpoint_id = self._selected_checkpoint_id()
        if self._selected_run_id and checkpoint_id and self.btn_overview_repair.isEnabled():
    ...
    def open_outputs(self):
        ...
        if out_dir and Path(out_dir).exists():
            ...
            msg_info(self, "Výstupy", "Výstupní adresář není evidován nebo již neexistuje. Archivované výstupy najdete na kartě Soubory.")
    ...
    def export_selected_run(self):
        ...
        try:
            ...
            archive = shutil.make_archive(str(target_path), "zip", root_dir=str(source))
