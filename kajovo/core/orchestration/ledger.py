"""Per-run budget ledger. Rezervuje celý placený požadavek před submittem."""
...
def _load(logger) -> dict[str, Any]:
    ...
    if value.get("version") != 1 or not isinstance(value.get("reservations"), dict):
        raise ContractError("Budget ledger má neplatný formát; placený submit je zablokován.")
    return value
...
def _reservation(payload: dict[str, Any], measurement: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "output_tokens": int(measurement.get("output_budget") or payload.get("max_output_tokens") or 0),
        "projected_cost_microusd": round(float(usd) * 1_000_000) if isinstance(usd, (int, float)) else None,
...
def _summarize(reservations: dict[str, Any]) -> dict[str, Any]:
    ...
    known_cost = [row["projected_cost_microusd"] for row in rows if row.get("pricing_known")]
    return {
        "paid_requests_reserved": len(rows),
        "input_tokens_reserved": sum(int(row.get("input_tokens") or 0) for row in rows),
        "output_tokens_reserved": sum(int(row.get("output_tokens") or 0) for row in rows),
        "projected_cost_microusd_known": sum(known_cost),
        "unknown_pricing_requests": sum(not row.get("pricing_known") for row in rows),
    }
...
def _check(summary: dict[str, Any], limits: dict[str, Any]) -> None:
    ...
    if summary["paid_requests_reserved"] > limits["max_paid_requests"]:
        blockers.append("Počet placených požadavků překračuje RUN_CONFIG_V2.max_paid_requests.")
    if summary["input_tokens_reserved"] > limits["max_input_tokens"]:
        blockers.append("Rezervovaný vstup překračuje RUN_CONFIG_V2.max_input_tokens.")
    if summary["output_tokens_reserved"] > limits["max_output_tokens"]:
        blockers.append("Rezervovaný výstup překračuje RUN_CONFIG_V2.max_output_tokens.")
    max_cost = limits["max_cost_microusd"]
    if max_cost is not None and summary["projected_cost_microusd_known"] > max_cost:
        blockers.append("Projektovaná známá cena překračuje RUN_CONFIG_V2.max_cost_microusd.")
    if summary["unknown_pricing_requests"] and limits["unknown_pricing"] == "block":
        blockers.append(
...
def reserve_paid_request(logger, cfg, client, payload: dict[str, Any], *,
                         measurement: dict[str, Any] | None = None,
                         batch: bool = False, key: str | None = None) -> dict[str, Any]:
    """Změří a rezervuje jeden požadavek. Stejný hash se nerezervuje podruhé."""
...
def reserve_batch(logger, cfg, client, requests: list[dict[str, Any]],
                  measurements: list[dict[str, Any]]) -> None:
    """Atomicky ověří celou dávku vůči zbývajícímu run budgetu před uploadem/submitem."""
