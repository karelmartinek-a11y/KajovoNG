...
def _bundle_if_present(run_dir):
    root = Path(run_dir)
...
def _sync_bundle_state(run_dir, state, event_type, data=None, *, seal=False):
    ...
    if isinstance(state.get("batch_id"), str) and state.get("batch_id"):
        batch_values.append(state["batch_id"])
    if isinstance(state.get("generate_batches"), dict):
        batch_values.extend(value for value in state["generate_batches"] if isinstance(value, str))
    status = str(state.get("status") or "unknown")
    bundle.update_run(
        {
            "status": status,
            "result_class": status if status in {"completed", "partial", "failed", "cancelled", "files_complete_unverified"} else "",
            "related_batch_ids": list(dict.fromkeys(batch_values)),
            "last_response_id": str(state.get("last_response_id") or bundle.run_record().get("last_response_id") or ""),
        }
    )
    bundle.append_event(
        event_type,
        dict(data or {}),
        source_module="batch_completion",
        operation="BATCH",
        human_message=str((data or {}).get("message") or ""),
        severity="error" if status in {"failed", "partial"} else "info",
    )
    if seal:
        bundle.seal()
...
def recover_unknown_submission(run_dir, records):
    ...
    if not input_file_id:
        raise ContractError(
            "Neurčitý submit nemá uložený input_file_id; automatický resubmit je zakázán."
        )
    ...
    if len(matches) != 1:
        raise ContractError(
            "Neurčitý submit odpovídá více vzdáleným dávkám; je nutný ruční zásah."
        )
    batch = matches[0]
    ...
    for path in sorted(Path(log_dir).glob("RUN_*/run_state.json"), reverse=True):
        ...
        for bid in dict.fromkeys([*work, *preflight_ids(state)]):
            info = dict(
                ...
                kind="work" if bid in work else "preflight",
            )
...
def import_bundle(
    ...
    try:
        ...
        return {
            "written": [],
            "hashes": dict(previous_hashes or {}),
            "errors": [str(exc)],
            "status": "partial",
        }
    if expected_ids is not None:
        ...
        if (
            not all(isinstance(cid, str) for cid in ids)
    ...
    for entry_index, entry in enumerate(entries, start=1):
        try:
            ...
            if contract == "C_FILES_ALL":
                ...
                bundles.extend(
                    (
                        safe_join_under_root(base, record["path"]),
                        record["content"],
                    )
                    for record in files
                )
                ...
                try:
                    ...
                    if (
                        chunk_index in info["parts"]
                        or count
                        and info["count"] not in (None, count)
                    ):
                        raise ContractError(
                            "Duplicitní část nebo rozdílný počet částí."
                        )
    ...
    for name, info in chunks.items():
        ...
        if (
            ...
            bundles.append(
                (
                    safe_join_under_root(target, name),
                    "".join(info["parts"][part] for part in range(count)),
                )
            )
            ...
            try:
                ...
                if Path(destination).exists():
                    ...
                    if old_hash != new_hash and old_hash != hashes.get(relative):
                        raise ContractError(
                            "Existující soubor byl změněn nebo nepatří importu; zachován."
                        )
...
def complete_saved_batch(client, run_dir, batch_id, settings, progress=None):
    ...
    if bundle:
        ...
        bundle.append_event(
            ...
            source_module="batch_completion",
            operation="BATCH",
            human_message=f"Zahájeno převzetí výsledků dávky {batch_id}.",
        )
    if progress:
        progress(
            ProgressEvent(
                "Kontrola stavu dávky",
                detail=f"Ověřuji {batch_id}.",
            )
        )
    ...
    if batch.get("status") not in TERMINAL:
        if bundle:
            ...
            bundle.append_event(
                ...
                source_module="batch_completion",
                operation="BATCH",
                human_message="Dávka ještě není v terminálním stavu.",
            )
        return {
    ...
    if state.get("generate_batch"):
        ...
        final_state = read_state(run_dir)
        _sync_bundle_state(
            run_dir,
            final_state,
            "batch.generate_import.completed",
            {
                "batch_id": batch_id,
    ...
    if progress:
        progress(
            ProgressEvent(
                "Stahování výsledků",
                detail="Stahuji výstupní a chybové JSONL.",
            )
        )
    for key in ("output_file_id", "error_file_id"):
        if batch.get(key):
            ...
            if bundle:
                bundle.archive_artifact(
                    raw_path,
                    role="batch_error" if key == "error_file_id" else "batch_output",
                    kind="batch_jsonl",
                    source="OpenAI Files API",
                    reusable=False,
    ...
    if bundle:
        ...
        bundle.record_validation(
            target_type="batch_import",
            target_id=batch_id,
            validator="import_bundle",
            status="failed" if result.get("errors") else "passed",
            errors=[str(value) for value in result.get("errors") or []],
            evidence={"status": result.get("status"), "written": result.get("written") or []},
        )
        if result.get("status") == "files_complete_unverified":
            bundle.checkpoint(
                "files_downloaded_validated",
                state_snapshot=state,
                safe_to_continue=True,
                reason="BATCH response byly staženy, kontrakty validovány a výstupní soubory zapsány.",
                invalidation_rules=[
                    "Změna nebo ztráta archivovaného výsledku invaliduje navazující použití.",
                ],
            )
    _sync_bundle_state(
        run_dir,
        ...
        "batch.import.completed",
        {
            ...
            "status": result.get("status"),
            "errors": result.get("errors") or [],
            "message": "Import dávky skončil a evidence byla aktualizována.",
        },
    )
    return result
