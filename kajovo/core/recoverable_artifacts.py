"""Přesné obsahově adresované artefakty oddělené od redigovaných logů."""
...
def save_artifact(run_dir, name, value):
    ...
    index = json.loads(index_path.read_text("utf-8")) if index_path.exists() else {"version": 1, "entries": {}}
    if index.get("version") != 1:
        raise ValueError("Nepodporovaná verze indexu artefaktů.")
    index["entries"][name] = digest
    atomic_write_text(str(index_path), json.dumps(index, ensure_ascii=False, sort_keys=True))
    return str(target)
...
def artifact_path(run_dir, name):
    ...
    if hashlib.sha256(target.read_bytes()).hexdigest() != digest:
        raise ValueError("Obnovitelný artefakt má neplatný hash; redigovaný log není náhrada.")
    return str(target)


STATE_ARTIFACTS = {"preparation_snapshot", "generate_batch", "generate_batches", "ui_state",
