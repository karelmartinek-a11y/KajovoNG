...
def test_missing_expected_file_does_not_overwrite_output(tmp_path):
    ...
    worker = CascadeRunExecutor(CascadeRunConfig("test", CascadeDefinition("test"), "", str(tmp_path)), AppSettings(), "test")
    worker.logger = Mock()
...
def test_cascade_exposes_effective_output_directory():
    ...
    worker = CascadeRunExecutor(CascadeRunConfig("test", definition, "", ""), AppSettings(), "test")
