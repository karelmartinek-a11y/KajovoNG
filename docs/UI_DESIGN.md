...
Produkční sestavu vytváří `kajovo.studio.application.create_window`; stejnou továrnu používají testy a snímkovací nástroj. Odstraněný `kajovo.desktop` není záložní cestou spouštěče ani referenční implementací. Kompatibilní `scripts/render_ui.py` deleguje na produkční `scripts/render_studio.py`, takže snímkování ověřuje skutečné Studio. Samostatný převodník používá Qt a společné komponenty.
