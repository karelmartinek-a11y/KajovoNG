...
3. Přidejte samostatné panely. Volba entity v nabídce vloží nedělitelný token do aktivního zadání. Kopírování uvnitř aplikace zachová strukturu; prostý text nezískává oprávnění odkazovat na entitu. Každý panel má vlastní formát.
4. Vyberte panely a zvolte Vygenerovat vybrané panely. Historie ukazuje skutečné stavy a počty. Po zavření aplikace pokračuje dávka u poskytovatele; po otevření se obnoví místní sledování. Po vypršení místního sledovacího limitu použijte Obnovit / převzít.
5. Text přidejte samostatně v náhledu. Bublinu lze přesouvat, měnit její velikost, hrot, písmo a přesné znění. Text, který se nevejde, blokuje export. SFX povoluje globální volba stylu. Kresba se objednává bez textu; náhled a export používají stejný Qt renderer a font Montserrat s diakritikou.
6. Upravit kresbu vytvoří samostatnou obrazovou dávku s poslední schválenou kresbou jako prvním vstupem. Výsledek je kandidátní verze. Použít vybranou verzi ji schválí; stejné tlačítko vrací starší verzi. Změna textu nebo plátna také vytváří verzi bez placeného generování. Export ukládá jednotlivý panel, nikoli stránku.
...
Run Bundle používá režim `COMIC`, ID operace a cestu knihovny. Dávky a Historie mohou otevřít komiksovou evidenci. Usage a doložitelné obrazové náklady jsou v evidenci jednotlivých výsledků; neúplný rozpad tokenů nebo neznámé cache složky znamenají neurčenou cenu, nikoli nulu. Ceník uchovává zdroj a okamžik ověření. Odhad není faktura poskytovatele.

## Datový model a migrace

První explicitní migrace `comic_store.SCHEMA` vytváří SQLite `user_version=1` a tabulku `migrations`. Neznámou neprázdnou databázi nepřepisuje. Nepoužívá dřívější provozní databázi aplikace. Tabulky:
...
- `assets`, `uploads`, `events`: neměnné soubory s SHA-256, účetně oddělená cache File IDs a audit.
...
| CML-C09 skutečné ovládání | `ComicsPage`, `Operations`, `ComicService` | `test_studio_comic_crud_and_panel_save`, `test_chip_selection_replacement_is_atomic` |
...
Testy jsou v `tests/test_comic_domain.py`, `test_comic_recovery.py` a `test_comic_ui.py`. Kontrola mockem dokládá integrační kontrakt, nikoli dostupnost API nebo vizuální konzistenci skutečného modelu. Živá akceptace je výslovně ruční pracovní scénář `scripts/accept_comic.py --workspace <oddělený adresář> --live`, vyžaduje oprávnění k placené práci a nikdy se nespouští z testů, CI, startu ani validačního tlačítka. Opakované spuštění obnovuje uložené operace. `backend_acceptance=completed` nenahrazuje vizuální kontrolu ani nativní UI akceptaci.
