...
Obnovené Zadání zobrazuje zdrojový běh, checkpoint, převzaté přípravné etapy, následující placenou etapu a počet ověřených souborů k přeskočení. Tlačítko pojmenovává následující etapu, například `Pokračovat od A2`. Změna zadání, režimu nebo kvality blokuje použití přípravného checkpointu. Po předání pracovníkovi zůstává přehled viditelný a stejné pokračování již nelze znovu spustit; další akce vyžaduje výběr v Historii nebo Nové zadání. Načtení jiného zadání ze souboru ruší původní pokračování.
