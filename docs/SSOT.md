...
Kruhová mapa v okně operace čte přímo `ProgressEvent`. Backend může oznámit známý plán událostí `PLAN` s `planned_steps`; plán nepotvrzuje zahájení ani dokončení kroků a nemění hodiny. Jmenovatelem kruhu je počet ohlášených plánovaných a skutečně pozorovaných etap, nikoli odhad času. Bez plánu zobrazuje jen pozorované etapy. Dynamické větvení může plán rozšířit. Zpracované položky s doloženým `completed`/`total` ukazuje zvlášť a poslední potvrzený počet zachová i po konci operace. Etapu označí za hotovou pouze její vlastní událost `completed`; `skipped` ani koncová zpráva `RUN` nepotvrzují dokončení. Čekání na ruční prostředek se nepočítá jako získaný výrobní cíl. Trvalá projekce etap je oddělená od limitu technického logu, takže dlouhé čekání nezahodí potvrzené kroky. Neznámý výsledek odeslání je blokovaný, bez automatického opakování. Neověřený krok zůstává neověřený i při ukončení místního pracovníka. Stáří poslední události není nová zpráva služby. Původní text zpráv se zobrazuje v otevřených technických podrobnostech, zatímco hlavní plocha používá srozumitelné stavové věty. Neznámý stav služby se nevydává za probíhající zpracování.

Produkční okno používá pouze `studio/progress_dialog.py` a `studio/progress_view.py` nad Qt-nezávislým `core/progress_model.py`. Konec vlákna bez výsledku není úspěch. Doprovodný chybový signál nepřepisuje doložené `submission_unknown`, `response_pending`, `cancelled` nebo `stopped` na obecné selhání. Tlačítko Zastavit předává žádost nejvýše jednou a neobnovuje čas poslední zprávy služby. Opakování operace resetuje celý zobrazovaný stav včetně počtů a metadat služby. Podrobnosti sémantiky, hranic a snímkování uvádí `docs/progress/implementation.md`.


U odpovědi na otázku oznamuje `_run_qa` samostatně přípravu podkladů (`QA_INPUT`), čekání a převzetí odpovědi (`QA_RESPONSE`) a kontrolu struktury tvrzení a jejich odkazů na dostupné podklady (`QA_VALIDATION`). `QA_INPUT completed` nastane až po sestavení a uložení požadavku, `QA_RESPONSE completed` až po kontrole formátu odpovědi a `QA_VALIDATION completed` až po kontrole `answer`/`claims`/`limitations`/`evidence_ids` a uložení výsledku. Chybný nebo blokovaný výsledek dokončení kontroly nehlásí. Kontrola existence evidence ID nepotvrzuje pravdivost tvrzení. Podrobnost runtime etap a vztah k referenčnímu návrhu uvádí `docs/progress/coverage.md`.
...
Příprava GENERATE/MODIFY oznamuje jednotlivě requirements, plán, SPINE a DETAIL; transport přebírá fázi ze skutečného WorkOrder. Dokončení přípravného kroku se oznamuje až po přijetí jeho kontraktu. Polling předává potvrzený stav služby a známé Response ID. Mapa odděluje plán od aktuální činnosti, v přehledu uvádí doložené dokončené etapy a zobrazuje následující známý krok. Technické zprávy přidává chronologicky s limitem 2000 bloků. Názvy kroků se zalamují. Ze samotného čekání neodvozuje dokončení parsování ani jiných mezikroků; dokončení jedné fáze neznamená dokončení celého běhu.
