...
U odpovědi na otázku oznamuje `_run_qa` samostatně přípravu podkladů (`QA_INPUT`), čekání a převzetí odpovědi (`QA_RESPONSE`) a věcnou kontrolu tvrzení proti dostupným podkladům (`QA_VALIDATION`). `QA_INPUT completed` nastane až po sestavení a uložení požadavku, `QA_RESPONSE completed` až po kontrole formátu odpovědi a `QA_VALIDATION completed` až po kontrole `answer`/`claims`/`limitations`/`evidence_ids` a uložení výsledku. Chybný nebo blokovaný výsledek dokončení kontroly nehlásí. Tyto etapy nejsou dosud úplným plánem všech variant odpovědi na otázku; stav pokrytí je v `docs/progress/coverage.md`.
