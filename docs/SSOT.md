...
Běžný běh vyžaduje klíč, model a neprázdné zadání. Před pracovním odesláním jádro provádí pouze deterministickou lokální validaci payloadu, pevné matice modelu, schémat, cest a kombinace voleb a podle potřeby ne-generativní čtení katalogu účtu nebo již existujících vzdálených prostředků. **Žádný samostatný placený preflight, testovací Response ani zkušební BATCH se před pracovní operací neposílá.** První generativní POST konkrétní etapy je její skutečný pracovní požadavek. GENERATE včetně Batch dovoluje samostatný model pro A1, A2 a A3; prázdná volba použije hlavní model. A0R používá model A1, A2Q model A2. MODIFY používá hlavní model ve všech B fázích. GENERATE a MODIFY včetně živé přípravy Batch navazují přes `previous_response_id`; dostupnost starší odpovědi lze ověřit ne-generativním načtením. Zadání delší než 150 000 znaků zavádějí přes technický A0 po 20 000 znacích. A0 potvrzuje příjem bez interpretace a nenahrazuje A0R/B0R. GENERATE přikládá uživatelské soubory v A0R; další přípravné fáze využívají návaznost. MODIFY připojuje vstupy do přípravných B fází. Dávkové A3/B3 používá samostatný snímek specifikace.
...
Souborové dávky GENERATE a MODIFY sdílejí evidenci `generate_batch`/`generate_batches`. Nový manifest verze 2 obsahuje `mode` a neměnný snímek zadání, requirements, plánu, struktury a `maximum_quality`. GENERATE vytváří úlohy A3_FILE, MODIFY úlohy B3_FILE pro textové položky `touched_files`, s pevnou akcí `add`/`modify`, úplným původním obsahem a textovým kontextem jejich závislostí. Instrukce obsahují CORE a příslušnou fázi; schéma se vynucuje pouze přes `text.format`. Import zachovává kompatibilitu s manifesty A3 verze 1 a historickými C_FILES_ALL dávkami.
...
GENERATE i MODIFY BATCH provede requirements, plán, strukturu a případný quality gate živě a po ověření specifikace vytvoří jeden řádek JSONL na každý vybraný textový soubor A3_FILE/B3_FILE. Řádek obsahuje `custom_id`, `method: POST`, `url: /v1/responses` a `body`. Dávkové A3/B3 nepoužívá `previous_response_id` ani nástroje; má společné zadání, requirements, plán, kanonickou strukturu a Maximum Quality. B3 přidává `original_content` a přesnou akci `add`/`modify`; změna vyžaduje úplný původní text ze schváleného IN. `C_FILES_ALL` zůstává pouze pro kompatibilní import starších dávek, nové běhy jej nepoužívají. JSONL se před síťovým uploadem kompletně lokálně validuje a následně se nahrává s účelem `batch` jako skutečný vstup pracovní dávky. Stav `batch_pending` znamená odeslání pracovní dávky, nikoli dokončení generování.
...
`generate_batch` je sdílený klíč evidence GENERATE i nového MODIFY BATCH. Manifest má `version: 2` a `mode: GENERATE` nebo `MODIFY`, neměnný `snapshot` se SHA-256, požadavky a mapování ID na cesty. Verze 1 zůstává čitelná jako GENERATE. Kontrola verze 2 porovnává také souborovou specifikaci, kontrakt, cestu a u MODIFY původní text a akci. Jedna dávka má jediný souborový model, 1 až 50 000 položek a nejvýše 200 MB JSONL. Definitivní přijetí konkrétního modelu ověřuje až skutečný pracovní požadavek služby; lokální pravidlo připouští pouze známé podporované textové rodiny. Každý A3_FILE/B3_FILE musí mít index 0, počet částí 1, `has_more: false` a `next_chunk_index: null`. Neúplný výstup se neukládá jako hotový soubor.
...
Ruční opakování a oprava vybraných cest vytvářejí novou pracovní dávku bez opakování requirements, plánu, struktury a quality gate, se stejným snímkem a režimem. Oprava přidává připomínku a aktuální obsah. Ani opakování, ani oprava nevytváří před vlastní pracovní dávkou pomocnou zkušební dávku. `generate_batches` a `batch_imports` zachovávají vztahy a výsledky přes restart. Starý ReRun bez společné specifikace není podkladem hybridní dávky.
