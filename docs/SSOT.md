...
A3_FILE a B3_FILE obsahují označení `contract`, požadovanou `path`, textový `content` a objekt `chunking`; B3 také akci změny. LIVE i BATCH používají stejný kontrakt jednoho úplného souboru: `chunk_index=0`, `chunk_count=1`, `has_more=false`, `next_chunk_index=null`. Model neřídí transportní dělení souboru a žádná další response není pokračováním souboru. Pokud se očekávaný úplný soubor nevejde do maximálního výstupu modelu, příprava musí odpovědnost rozdělit do menších souborů místo skládání modelových chunků. JSON Schema i následná lokální validace vynucují tentýž kontrakt. Všechny lokální vady contract/path/action/content/chunking i zákaz nepovoleného prázdného souboru jsou uvnitř nejvýše tří pokusů stejné repair smyčky. `incomplete/max_output_tokens` smí vyvolat nový úplný pokus pouze s vyšším výstupním rozpočtem do pevného maxima modelu; po dosažení maxima běh selže s požadavkem opravit A2. Neurčitý síťový submit se tímto mechanismem neopakuje. QFILE používá tentýž jednorázový A3_FILE kontrakt; SEND AS BATCH pro něj není dostupné.
